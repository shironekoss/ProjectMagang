                        ...
                        foreach ($kit["IsiKit"] as $isikit) {
                            if (isset($kit["siteID"])) {
                                $available = DB::connection('sqlsrv')
                                    ...
                                    ->where('ITEMKITMAINTENANCE.Component Item Number', $isikit["kode_komponen"])
                                    ->pluck("QTYONHND")
                                    ->first();
                                // tanda . di sqlserver menandakan nilai 0
